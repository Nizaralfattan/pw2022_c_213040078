...
<html lang="en">
  ...
  <tbody>
    <?php $no = 1; foreach($mahasiswa as $key => $mhs) { ?>
              <tr>
                <th scope="row"> <?= $no++; ?> </th>
                <td>
	 	              	<img src="img/<?= $mhs['gambar'] ?>" class = "rounded-circle" height = "50px" >
                </td>
                  <td><?= $mhs['nama'] ?></td>
                <td>
                  <a href="#" class="btn btn-sm text-white bg-warning">Edit</a>
                  <a href="#" class="btn btn-sm text-white bg-danger">Delete</a>
                  <a href="kuliah_latihan3.php?id=<?= $key; ?>" class="btn btn-sm text-white bg-info">Detail</a>
                </td>
              </tr>
              <?php } ?>
  </tbody>
